...
DATASET_DIR = (
    Path(os.environ.get("DATASET_DIR", _SCRIPT_DIR / "pc-part-dataset"))
    / "data"
    / "json"
)
...
class DatasetLoader:
    ...
    def search(self, filename: str, query: str) -> list[PartMatch]:
        ...
        scored.sort(
            key=lambda x: (-x[0], x[1].price if x[1].price is not None else 1e9)
        )
    ...
    def top(self, filename: str) -> list[PartMatch]:
        ...
        for rec in records[:MAX_SEARCH_RESULTS]:
            ...
            price = float(price_raw) if price_raw is not None else None
            results.append(
                PartMatch(name=rec.get("name", "Unknown"), price=price, data=rec)
            )
        return results
...
class CompatibilityChecker:
    ...
    def __init__(self, searches: list[ComponentSearch]):
        ...
        self._best: dict[str, Optional[PartMatch]] = {
            cs.category: (cs.matches[0] if cs.matches else None) for cs in searches
        }
    ...
    def _ram_type(self) -> None:
        ...
        """
        ram_cs = next(
            (cs for cs in self._searches if cs.category == "Memory (RAM)"), None
        )
        mb_cs = next(
            (cs for cs in self._searches if cs.category == "Motherboard"), None
        )
        ...
        if ram_p:
            ram_ddr = _ddr_gen_from_speed(ram_p.data.get("speed")) or _ddr_gen_from_str(
                ram_p.name
            )
        ...
        if mb_p:
            mb_ddr = _ddr_gen_from_str(mb_p.name) or _ddr_gen_from_str(
                str(mb_p.data.get("memory_type", ""))
            )
    ...
    def _case_form_factor(self) -> None:
        ...
        case_r = max(
            (v for k, v in SIZE.items() if k in _norm(str(case_ff))), default=None
        )
        if mb_r and case_r:
            if mb_r > case_r:
                self._add(
    ...
    def _psu_wattage(self) -> None:
        ...
        parts = []
        if cpu_tdp:
            parts.append(f"CPU ~{int(cpu_tdp)}W")
        if gpu_tdp:
            parts.append(f"GPU ~{int(gpu_tdp)}W")
        parts.append(f"overhead ~{self._PLATFORM_W}W")
        summary = " + ".join(parts) + f" = ~{int(total)}W"
        if psu_w < total:
            self._add(
                ERROR,
                ["CPU", "Video Card (GPU)", "Power Supply (PSU)"],
    ...
    def _cooler_socket(self) -> None:
        ...
        if _norm(str(cpu_s)) in _norm(str(cooler_s)):
            self._add(
                INFO, ["CPU", "CPU Cooler"], f"Cooler socket OK — supports {cpu_s}."
            )
        else:
            self._add(
                WARNING,
...
def search_dataset(
    loader: DatasetLoader, preferences: dict[str, str]
    ...
    for component, preference in preferences.items():
        ...
        try:
            matches = (
                loader.search(filename, preference)
                if preference
                else loader.top(filename)
            )
            if not matches:
...
def _compat_for_gpt(issues: list[CompatibilityIssue]) -> str:
    ...
    return "\n".join(
        f"[{i.severity}] ({', '.join(i.components)}) {i.message}" for i in issues
    )
    ...
    for cs in searches:
        ...
        for i, m in enumerate(cs.matches, 1):
            ...
            specs = [
                k
                for k in m.data
                if k not in ("name", "price") and m.data[k] is not None
            ]
...
def build_full_prompt(
    ...
    goal = (
        "If a better part exists within budget — better performance, better value, better compatibility — recommend it instead and briefly explain why it's the superior choice."
        if mode == "a PC build "
        else "You should recommend new parts within the given budget with the goal of maximizing performance, value, and compatibility. This is an upgrade to their current build so recommendations should be given keeping in mind their current build and the fact that their budget excludes these parts as they already own them."
    )
    est = (
        ""
        if is_full_build
        else ", only sum the price of the components that are different from the users current components"
    )
...
def main():
    ...
    prompt = build_full_prompt(
        preferences, budget, use_case, dataset_blk, compat_block, "Full PC build"
    )
    ...
    save = (
        input("\nWould you like to save these recommendations to a file? (y/n): ")
        .strip()
        .lower()
    )
    if save == "y":
        ...
        with open(filename, "w", encoding="utf-8") as f:
            ...
            for cs in searches:
                f.write(
                    f"[{cs.category}]  Preference: {cs.user_preference or 'None'}\n"
                )
                if cs.error:
...
def compatibility_endpoint(req: CompatibilityRequest):
    ...
    issues = _filter_issues_for_selected(
        run_compatibility_check(searches), selected_components
    )
...
def run_endpoint(req: RunRequest):
    ...
    prompt = build_full_prompt(
        preferences, budget, use_case, dataset_blk, compat_block, mode
    )
